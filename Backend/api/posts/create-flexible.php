...
try {
    ...
    if (strpos($contentType, 'application/json') !== false) {
        ...
        if (isset($input['tags']) && is_string($input['tags'])) {
            $input['tags'] = json_decode($input['tags'], true) ?: [];
        }
        
        // Parse categories if they're JSON string
        if (isset($input['categories']) && is_string($input['categories'])) {
            $input['categories'] = json_decode($input['categories'], true) ?: [];
        }
        
        // Handle category_id field (convert to categories array)
        if (isset($input['category_id']) && !isset($input['categories'])) {
            $input['categories'] = [intval($input['category_id'])];
        }
    }
